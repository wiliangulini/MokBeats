...
describe('LoginComponent', () => {
    ...
    beforeEach(async () => {
        const authServiceSpy = {
            registrar: vi.fn().mockName("AuthService.registrar"),
            fazerLogin: vi.fn().mockName("AuthService.fazerLogin"),
            loginComGoogle: vi.fn().mockName("AuthService.loginComGoogle")
        };
        authServiceSpy.registrar.mockReturnValue(of({}));
        authServiceSpy.fazerLogin.mockReturnValue(of({}));
        activeModal = {
    ...
    it('deve mapear Mok Starters e Mok Makers para comprador/produtor', () => {
        ...
        expect(component.isCustomSelectOpen('tipoPerfil')).toBe(false);
    ...
    it('deve enviar tipoPerfil no payload sem alterar contrato no onSubmit', () => {
        ...
        const payload = vi.mocked((authService.registrar as Mock)).mock.lastCall![0];
    ...
    it('deve manter apenas um select custom aberto por vez', () => {
        ...
        expect(component.isCustomSelectOpen('typePerson')).toBe(true);
        expect(component.isCustomSelectOpen('tipoPerfil')).toBe(false);

        component.toggleCustomSelect('tipoPerfil');
        expect(component.isCustomSelectOpen('typePerson')).toBe(false);
        expect(component.isCustomSelectOpen('tipoPerfil')).toBe(true);
    ...
    it('deve aplicar classe open somente no select ativo para controlar empilhamento', () => {
        ...
        const perfilSelect = fixture.nativeElement.querySelector('[data-select-key="tipoPerfil"]');
        expect(pessoaSelect.classList.contains('open')).toBe(true);
        expect(perfilSelect.classList.contains('open')).toBe(false);

        component.toggleCustomSelect('tipoPerfil');
        fixture.detectChanges();

        expect(pessoaSelect.classList.contains('open')).toBe(false);
        expect(perfilSelect.classList.contains('open')).toBe(true);
    });
});
