...
describe('WaveSurferTestComponent behavior', () => {
    ...
    beforeEach(async () => {
        ...
        vi.spyOn(component as any, 'initWaveSurfer').mockImplementation(() => {
            (component as any).wavesurfer = fake as any;
        });
    });

    it('syncs time with currentTime$ when current track', async () => {
    ...
    it('does not move time when not current track', async () => {
        fixture.detectChanges();
    ...
    it('requests seek from minimap click when current track', () => {
        ...
        component.ngOnInit();
        service.setCurrentMusicID(1);
        vi.spyOn(service, 'requestSeek').mockReturnValue(undefined);
